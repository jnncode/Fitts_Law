...
class QuestionPage(Frame):
    def __init__(self, master=None):
        ...
        # messagebox.showinfo('Participant ID: ', f'{id}') # display for user
        id_label = Label(self, text="ID").grid(row=1, column=1, sticky="nsew")
        id_entry = Entry(self)
        # id_entry.insert(0, generate_id)
        id_entry.grid(row=1, column=2, sticky="nsew") 
        # addIdEntry()
        ...
        #     if text == '':
        #         # If the entry widget is empty, insert the new data
        #         id_entry.insert(0, generate_id)
        #     else:
        #     # If the entry widget already has some text, replace it with the new data
        #         id_entry.delete(0, 'end')
        #         id_entry.insert(0, generate_id)
        
        age_label = Label(self, text="Age").grid(row=3, column=1, sticky="nsew")  
        age_entry = Entry(self)
        age_entry.grid(row=2, column=1, sticky="nsew")  

        gender = Label(self, text="Gender").grid(row=5, column=1, sticky="nsew")  
        gender_entry = Entry(self)
        gender_entry.grid(row=4, column=1, sticky="nsew") 

        hand_label = Label(self, text="Handedness").grid(row=7, column=1, sticky="nsew")  
        hand_entry = Entry(self)
        hand_entry.grid(row=6, column=1, sticky="nsew")
        ...
        def validate():
            ...
            try:
                ...
                value2 = int(id_entry.get())
                if value1 and value2 < 0 or value1 and value2 > 100:
                    raise ValueError
            except ValueError:
...
class InstructionPage(Frame):
    def __init__(self, master=None):
        ...
        label_instruction = Message(self, text = "Instructions\n\n\n" + 
...
#                 print("Click Interals:", click_intervals)
#                 self.destroy()
